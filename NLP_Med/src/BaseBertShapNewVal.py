...
# Функция для предсказаний (для SHAP)
def model_predict(texts, model, tokenizer, device):
...
def main():
    ...
    # SHAP визуализация только если модель достаточно хороша
    if best_f1 > 0.5:  # Порог качества
        try:
            print("\nГенерация выгрузки SHAP...")
            explainer = shap.Explainer(
                lambda texts: model_predict(texts, model, tokenizer, device),
                tokenizer
            )
            
            sample_idx = np.random.choice(len(val_texts), size=min(3, len(val_texts)), replace=False)
            shap_values = explainer([val_texts[i] for i in sample_idx])
            ...
            for i, idx in enumerate(sample_idx):
                ...
                print(f"True: {id2label[val_labels[idx]]}")
                print(f"Predicted: {id2label[np.argmax(shap_values[i].values)]}")
                shap.plots.text(shap_values[i])
        except Exception as e:
            print(f"\nSHAP ошибка визуализации: {str(e)}")
    else:
